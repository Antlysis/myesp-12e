...
Adafruit_MQTT_Publish lastwill = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "connectivity/2909e5f09f5b57daf687565a8e9a48e6");

// Notice MQTT paths for AIO follow the form: <username>/feeds/<feedname>
Adafruit_MQTT_Publish runStatus = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "Lock/2909e5f09f5b57daf687565a8e9a48e6");
Adafruit_MQTT_Publish detStatus = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "detDrop/2909e5f09f5b57daf687565a8e9a48e6");
Adafruit_MQTT_Publish coinIn = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "coinIn/2909e5f09f5b57daf687565a8e9a48e6");
Adafruit_MQTT_Publish versFeed = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "versionFeed/2909e5f09f5b57daf687565a8e9a48e6");
// Setup a feed called 'esp8266_led' for subscribing to changes.
Adafruit_MQTT_Subscribe receivedPayment = Adafruit_MQTT_Subscribe(&mqtt, MQTT_USERNAME "2909e5f09f5b57daf687565a8e9a48e6");
...
String machineType = String("dexDryerSingle");
...
void setup() {
  ...
  mqtt.will(MQTT_USERNAME "connectivity/2909e5f09f5b57daf687565a8e9a48e6", "OFF");
  timerInit();
}
...
void loop() {
  ...
  if (machineType == "ipsoWasher") {
    ...
    payIpsoD_NO(&epay, &lastMilli, &currMilli);
    while ((subscription = mqtt.readSubscription())) {
      if (subscription == &receivedPayment) {
        char *message = (char *)receivedPayment.lastread;
        float to_float = atof(message);
    ...
    payDexW_NO(&epay, &lastMilli, &currMilli);
    while ((subscription = mqtt.readSubscription())) {
      if (subscription == &receivedPayment) {
        char *message = (char *)receivedPayment.lastread;
        float to_float = atof(message);
    ...
    payDexD_NO(&epay, &lastMilli, &currMilli);
    while ((subscription = mqtt.readSubscription())) {
      if (subscription == &receivedPayment) {
    ...
    payDet_NO(&epay, &lastMilli, &currMilli);
    while ((subscription = mqtt.readSubscription())) {
      if (subscription == &receivedPayment) {
        char *message = (char *)receivedPayment.lastread;
        float to_float = atof(message);
...
void payDexW_NO (unsigned int *amt, unsigned int *lastms, unsigned int *currms ) {
  unsigned int jdo = 0;
  unsigned int jdo2 = 0;
  if (*amt != 0) {
    ...
    } else if (*currms - *lastms >= 100 && *currms - *lastms <= 650) {
      if (jdo2 == 0) {
        digitalWrite(LED_BUILTIN, LOW);
        digitalWrite(DEX_W_CTRL, LOW);
        jdo2++;
...
void payDexD_NO (unsigned int *amt, unsigned int *lastms, unsigned int *currms ) {
  unsigned int jdo = 0;
  unsigned int jdo2 = 0;
  if (*amt != 0) {
    if (*currms - *lastms <= 100) {
...
void payIpsoD_NO (unsigned int *amt, unsigned int *lastms, unsigned int *currms ) {
  unsigned int jdo = 0;
  unsigned int jdo2 = 0;
  if (*amt != 0) {
    if (*currms - *lastms <= 50) {
...
void payDet_NO (unsigned int *amt, unsigned int *lastms, unsigned int *currms ) {
  unsigned int jdo = 0;
  unsigned int jdo2 = 0;
  if (*amt != 0) {
    if (*currms - *lastms <= 100) {
...
void payBill_NO (unsigned int *amt, unsigned int *lastms, unsigned int *currms ) {
  unsigned int jdo = 0;
  unsigned int jdo2 = 0;
  if (*amt != 0) {
    if (*currms - *lastms <= 50) {
