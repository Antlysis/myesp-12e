...
#define DEX_D_IN 4         //JP6 - 5
#define IPSO_D_IN 4        //JP6 - 5
#define DEX_W_IN 4         //JP6 - 5
#define DEX_D1_STATUS 5    //JP5 - 4
#define DEX_W_STATUS 5     //JP5 - 4
#define IPSO_D_STATUS 5    //JP5 - 4
#define IPSO_D_CTRL 2      //JP4
#define DEX_D_CTRL  2      //JP4
#define DEX_W_CTRL  2      //JP4
#define BILL_IN     10     //JP8     
...
#define MQTT_SERVER      "192.168.1.25" // give static address
...
String machineType = String("dexDryer");
...
// ipsoWasher | ipsoDryer | dexWasher | dexDryer | detergent | billAcceptor 
...
void loop() { 
 ...
 // connection and automatically reconnect when disconnected).  See the MQTT_connect 
  ...
  if (machineType == "ipsoWasher") {
    ...
    coinCount(coin_input1, &coinCounter1, &high1);
    ...
    coinCount(coin_input1, &coinCounter1, &high1);
    dexdryerStatusRead(lockState1, lockState2, &SendOne1, &SendOne2, &checkOne1, &checkOne2, &lockCounter1, &lockCounter2, &unlockCounter1, &unlockCounter2);
    Adafruit_MQTT_Subscribe *subscription;
    while ((subscription = mqtt.readSubscription())) { 
      if (subscription == &receivedPayment) {
        char *message = (char *)receivedPayment.lastread; 
...
void coinCount(int coin_input, int *coinCounter, int *high) {
  if (*high == 0) {
    if(coin_input == HIGH) {
...
void dexdryerStatusRead(int lockState1, int lockState2, int *SendOne1, int *SendOne2, int *checkOne1, int *checkOne2, int *lockCounter1, int *lockCounter2, int *unlockCounter1, int *unlockCounter2) {
 if (lockState1 == LOW) {
    if (*SendOne1 <= 5) {
      (*lockCounter1)++;
        if(*lockCounter1 > 700){
