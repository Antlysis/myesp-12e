...
#define MQTT_SERVER      "192.168.1.102" // give static address
...
const char* fwUrlBase = "http://192.168.1.102/fw/";

// Create an ESP8266 WiFiClient class to connect to the MQTT server. 
WiFiClient client; 
 //Setup the MQTT client class by passing in the WiFi client and MQTT server and login details. 
Adafruit_MQTT_Client mqtt(&client, MQTT_SERVER, MQTT_PORT, MQTT_USERNAME, MQTT_PASSWORD); 
Adafruit_MQTT_Publish lastwill = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "connectivity/19c1bd29f764c537a988aa5bfbbad44f");

// Notice MQTT paths for AIO follow the form: <username>/feeds/<feedname> 
Adafruit_MQTT_Publish runStatus = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "Lock/19c1bd29f764c537a988aa5bfbbad44f");
Adafruit_MQTT_Publish detStatus = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "detDrop/19c1bd29f764c537a988aa5bfbbad44f");
Adafruit_MQTT_Publish coinIn = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "coinIn/19c1bd29f764c537a988aa5bfbbad44f");
Adafruit_MQTT_Publish versFeed = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "versionFeed/19c1bd29f764c537a988aa5bfbbad44f");
// Setup a feed called 'esp8266_led' for subscribing to changes. 
Adafruit_MQTT_Subscribe receivedPayment = Adafruit_MQTT_Subscribe(&mqtt, MQTT_USERNAME "19c1bd29f764c537a988aa5bfbbad44f");
...
void setup() {
 ...
 mqtt.will(MQTT_USERNAME "connectivity/19c1bd29f764c537a988aa5bfbbad44f", "OFF");
 timerInit();
...
void coinCount(int coin_input, int *coinCounter, int *high, int *strt) {
  if (*strt == 0) {  
    if (*high == 0) {
      if(coin_input == HIGH) {
